...
void MainWindow::on_actionShow_note_triggered()   {if(!note.empty())ui->textEdit->setText(note[num]);}
void MainWindow::on_actionAdd_in_home_triggered() {if(!note.empty())home.push_back(note[num]);}
...
void MainWindow::on_actionNext_triggered()        {if(!note.empty())ui->textEdit->setText(note[num<note.size()-1?++num:num]);}
